...
QMHtml.VideoChat = {
  onCallTpl(params) {
    return _.template(
      `${
        '<div class="incoming-call l-flexbox l-flexbox_column l-flexbox_flexbetween">' +
        '<div class="incoming-call-info l-flexbox l-flexbox_column">' +
        '<div class="message-avatar avatar info-avatar" style="background-image:url('
      }${params.userAvatar})"></div>` +
  ...
  showError() {
    ...
    const $html = $(
      '<article class="message message_service l-flexbox l-flexbox_alignstretch">' +
...
QMHtml.User = {
  contactPopover(params, roster) {
    const $html = $(
      '<ul class="list-actions list-actions_contacts popover j-listActionsContacts"></ul>'
    );
    let htmlStr = '';

    if (params.dialogType === 3 && roster && roster.subscription !== 'none') {
      htmlStr =
        '<li class="list-item"><a class="videoCall list-actions-action" data-id="<%=ids%>" href="#">Video call</a></li>' +
      ...
      htmlStr =
        '<li class="list-item"><a class="list-actions-action addToGroupChat" data-group="true" data-ids="<%=occupantsIds%>" ' +
        'data-dialog="<%=dialogId%>" href="#">Add people</a></li>';
    }

    if (params.dialogType === 3) {
      htmlStr +=
        '<li class="list-item"><a class="j-deleteContact deleteContact list-actions-action" href="#">Delete contact</a></li>' +
        '<li class="list-item"><a class="list-actions-action userDetails" data-id="<%=ids%>" href="#">Profile</a></li>';
    }

    htmlStr +=
      '<li class="list-item"><a class="j-deleteChat deleteChat list-actions-action" data-group="true" href="#">Remove chat</a></li>';
  ...
  occupantPopover(params, roster) {
    const $html = $(
      '<ul class="list-actions list-actions_occupants popover j-listActionsContacts"></ul>'
    );
    let htmlStr = '';

    if (!roster || (roster.subscription === 'none' && !roster.ask)) {
      htmlStr =
        '<li class="list-item j-listItem" data-jid="<%=jid%>">' +
      ...
      htmlStr =
        '<li class="list-item"><a class="list-actions-action videoCall" data-id="<%=id%>" href="#">Video call</a></li>' +
  ...
  getControlButtonsForPopupDetails(roster) {
    ...
    htmlStr +=
      '<div class="btn_userDetails <%=roster%> writeMessage">' +
  ...
  profilePopover() {
    return $(
      '<ul class="list-actions list-actions_profile popover">' +
...
QMHtml.Messages = {
  setMap(params) {
    const linkTemplate = '<a class="open_map" href="<%=mapLink%>" target="_blank"></a>';
    const htmlTemplate = _.template(linkTemplate)(params);
